...
Apply a function to 1-D slices of an array along the given axis. The function is called once for each slice, and the results are assembled into an output array. Supports arrays of any dimensionality. For ND arrays, the function iterates over all combinations of indices on dimensions other than `axis`. Any trailing arguments are forwarded to each invocation of `func1d`, matching NumPy's `np.apply_along_axis(func1d, axis, arr, *args)`. Existing 3-argument calls behave identically.

```typescript
function apply_along_axis(
  func1d: (arr: NDArray, ...args: unknown[]) => NDArray | number,
  axis: number,
  arr: ArrayLike,
  ...args: unknown[]
): NDArray
...
|-|-|-|-|
| `func1d` | `(arr: NDArray, ...args) => number \| NDArray` | -- | Function that operates on a 1-D array and returns a scalar or 1-D array. Receives any trailing `...args`. |
| `axis` | `number` | -- | Axis along which to apply `func1d`. |
| `arr` | `ArrayLike` | -- | Input array. |
| `...args` | `any[]` | -- | Additional arguments forwarded to each call of `func1d`. |

**Returns:** `NDArray` -- The result of applying `func1d` along the specified axis.

<Warning>
Changed in v1.7.0: `func1d` now receives a **view** of each slice rather than a copy, matching NumPy. A callback that mutates its argument in place will write through to `arr`. If your callback modifies the slice it is handed, copy it first:

```typescript
apply_along_axis((s) => { const t = s.copy(); t.set(0, 0); return t; }, 0, a);
```
</Warning>
...
// Sum along axis 2 of a 3-D array
const b = np.ones([2, 3, 4]);
...
// Forward extra args to func1d (here: a scalar multiplier).
const scaled = apply_along_axis(
  (row, k) => row.multiply(k),
  1,
  a,
  3
);
...
Set the size of the WASM linear memory before it is allocated. Raise `maxMemory` when a workload exhausts the heap, or lower it in memory-constrained environments such as a serverless runtime.

```typescript
function configureWasm(options: ConfigureWasmOptions): void
```

| Name | Type | Default | Description |
|-|-|-|-|
| `options.maxMemory` | `number` | `268435456` (256 MiB) | Total WASM linear memory, in bytes. Must be positive. |
| `options.scratchSize` | `number` | `maxMemory / 16` | Scratch region for temporary kernel buffers, in bytes. |

**Returns:** `void`

<Warning>
This must be called before any array operation. WASM memory is allocated lazily on first use, and once allocated it cannot be resized -- calling `configureWasm` afterwards throws.
</Warning>

```typescript
import * as np from 'numpy-ts';

// Before touching any array API
np.configureWasm({ maxMemory: 1024 * 1024 * 1024 }); // 1 GiB

const a = np.zeros([10000, 10000]);
```
...
Report how many bytes remain available in the WASM heap. Useful for spotting a leak in a long-running process, or for sizing a batch before allocating it.

```typescript
function wasmFreeBytes(): number
```

**Returns:** `number` -- Free bytes in the WASM heap. Initializes the heap if it has not been allocated yet, so calling this counts as first use for the purposes of `configureWasm`.
...
console.log(before - np.wasmFreeBytes()); // bytes consumed by `a`
```
