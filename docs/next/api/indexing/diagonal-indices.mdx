...
| `mask_func` | `(m: NDArray, k: number) => NDArray` | -- | A function that accepts `(m, k)` -- an `n`x`n` array and a diagonal offset -- and returns a 2-D boolean or integer array (like `triu` or `tril`). |
