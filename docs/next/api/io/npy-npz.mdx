...
```typescript
function loadNpzFile(path: string, options?: NpzParseOptions): Promise<NpzParseResult>
```
...
Every function above has a synchronous counterpart taking the same arguments and returning the value directly rather than a `Promise`. They are Node-only -- they read and write through `node:fs` -- so use the async forms in the browser.

```typescript
function loadSync(path: string, options?: LoadOptions): NDArray | NpzParseResultNDArray

function loadNpySync(path: string): NDArray

function loadNpzFileSync(path: string, options?: NpzParseOptions): NpzParseResultNDArray

function saveSync(path: string, arr: NDArray): void

function saveNpySync(path: string, arr: NDArray): void

function saveNpzFileSync(path: string, arrays: NpzArraysInput): void
```

<Note>
`saveNpzFileSync` takes no options argument. Compression is only available through async `saveNpzFile`, for the reason given below.
</Note>
