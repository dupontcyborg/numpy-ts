...
Every file function above has a synchronous counterpart taking the same arguments and returning the array directly rather than a `Promise`. They are Node-only -- they read and write through `node:fs` -- so use the async forms in the browser.

```typescript
function loadtxtSync(path: string, options?: LoadTxtOptions): NDArray

function savetxtSync(path: string, arr: NDArray, options?: SaveTxtOptions): void

function genfromtxtFileSync(path: string, options?: LoadTxtOptions): NDArray

function fromregexFileSync(path: string, regexp: RegExp | string, dtype?: DType): NDArray
```
