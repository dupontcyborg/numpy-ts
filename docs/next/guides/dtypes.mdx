...
### Caveat: `NaN` / `±Inf` → integer

NumPy 2.x **explicitly documents** the float-to-integer conversion of `NaN`, `+Inf`, and `-Inf` as **platform-dependent**:

- On x86, the `cvttsd2si` instruction returns the "indefinite integer" — `INT_MIN` for signed targets.
- On arm64, the `fcvtzs` instruction returns `0`.

Because NumPy itself produces different results on different CPUs *for the same input and same version*, no implementation can match both at once. numpy-ts picks the arm64 convention as a stable, predictable rule across all runtimes:
...
| `-Inf` | signed | target min (e.g. `-128` for int8) |
| `-Inf` | unsigned | `0` |

<Tip>
If you cast `NaN` / `±Inf` to an integer dtype in code that's compared against a NumPy reference, expect divergence on x86 NumPy. Filter or replace these values explicitly (e.g. `np.isnan` + masked assignment) before casting if cross-platform parity matters.
</Tip>
