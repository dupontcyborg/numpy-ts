...
const data = np.arange(1000000);  // 1 million elements
...
console.log(b.flags.OWNDATA);  // false -- b is a view
console.log(b.base === a);     // true  -- b shares a's data

const c = np.flatten(a);
console.log(c.flags.OWNDATA);  // true  -- c owns its data
```
...
const bigArray = np.random.random([10000, 100]);
...
console.log('Batch means shape:', allMeans.shape); // [10, 100]
```
...
const r = np.ravel(a);
console.log(r.flags.OWNDATA); // false

// After transpose, the array is no longer contiguous
const t = a.T;
const r2 = np.ravel(t);
console.log(r2.flags.OWNDATA); // true (had to copy)
```
...
```typescript
// Full entry point: ~200-300 KB, all functions included, method chaining
import * as np from 'numpy-ts';
...
```typescript
// This imports only ~15 KB of code
...
const a = np.random.random([100000]);
...
console.log(rowMeansKept.shape);  // [2, 1]
...
```typescript
// Pattern: normalize along an axis
