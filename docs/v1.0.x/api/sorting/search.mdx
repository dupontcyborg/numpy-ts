...
```typescript
function searchsorted(a: ArrayLike, v: ArrayLike, side?: 'left' | 'right'): NDArray | number
```
...
**Returns:** `NDArray | number` -- Indices of insertion points. Returns a scalar `number` when `v` is a scalar, otherwise an `NDArray`.
...
```typescript
function extract(condition: ArrayLike, arr: ArrayLike): NDArray
```

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `condition` | `ArrayLike` | -- | Boolean array. Must be broadcastable to the shape of `arr`. |
| `arr` | `ArrayLike` | -- | Input array. |

**Returns:** `NDArray` -- 1D array of elements from `arr` where `condition` is `true`.
...
```typescript
function count_nonzero(a: ArrayLike, axis?: number | number[], keepdims?: boolean): number | NDArray
```

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `a` | `ArrayLike` | -- | Input array. |
| `axis` | `number \| number[]` | `undefined` | Axis or axes along which to count. If `undefined`, counts over the entire array. |
| `keepdims` | `boolean` | `false` | If `true`, the reduced axes are kept as dimensions with size 1. |
