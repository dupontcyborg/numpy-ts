...
The order of dimensions in the result matches [numpy's behaviour](https://numpy.org/doc/stable/user/basics.indexing.html#combining-advanced-and-basic-indexing). I.e. if the advanced indices are continiguous, then they replace in-place, otherwise, they are transposed to be the first dimensions.
