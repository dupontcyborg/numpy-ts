...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `ary` | `ArrayLike` | -- | Input array. If multi-dimensional, it is flattened first. |
| `to_end` | `ArrayLike` | `undefined` | Values to append at the end of the returned differences. |
| `to_begin` | `ArrayLike` | `undefined` | Values to prepend at the beginning of the returned differences. |
...
| `...varargs` | `(number \| ArrayLike)[]` | `[]` | Spacing between sample points. Supports multiple formats: no arguments (unit spacing), a single scalar (uniform spacing for all axes), N scalars (one per axis), or N arrays (coordinates along each axis for non-uniform spacing). |
