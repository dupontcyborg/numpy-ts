...
canonical: https://numpyts.dev/latest/guides/roadmap
---
...
## Unified `intp` index dtype

Index-producing operations currently split across two `number`-yielding dtypes: `argmin`/`argmax`/`nanarg*` return `int32`, while `argsort`/`argwhere`/`flatnonzero`/`nonzero` return `float64`. The `int32` path overflows above ~2.1 billion (2³¹) elements, and the split is internally inconsistent.

We plan to introduce a single 53-bit `intp` dtype (backed by `Float64Array`, element access yields `number`) and route the whole index family through it — matching NumPy's `intp` semantics without the `bigint` ergonomics that a true `int64` index would force. This requires a new oracle-validated promotion rule and touches the index kernels, so it ships on its own rather than bundled with the dtype-typing work.
