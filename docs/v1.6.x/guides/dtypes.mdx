...
canonical: https://numpyts.dev/latest/guides/dtypes
---
...
Pass the dtype as a positional argument to any creation function. The dtype literal is also tracked in the return type — see [Compile-time dtype tracking](./typescript-patterns#compile-time-dtype-tracking):
...
These promotion rules are enforced at runtime **and** mirrored at the type level: `c` above is typed `NDArray<'float64'>`. The `Promote<A, B>` type helper exposes the rule directly. See [Compile-time dtype tracking](./typescript-patterns#compile-time-dtype-tracking).
...
### Reductions follow NumPy's result dtype
...
Floating-point reductions **preserve the input's float width** rather than always widening to `float64`. `mean`, `std`, `var`/`variance`, `median`, `percentile`, `quantile`, and the `nan*` family return `float32` for `float32` input and `float16` for `float16` input; `std`/`var` on `complex64` return `float32` (the real component):

```typescript
const a = np.array([[1, 2, 3], [4, 5, 6]], 'float32');
np.mean(a, 0).dtype;   // 'float32'  (was 'float64' before v1.6.0)
np.std(a, 0).dtype;    // 'float32'
np.median(a, 0).dtype; // 'float32'
```

<Warning>
  Changed in **v1.6.0**: these reductions previously always returned `float64`. They now match NumPy and preserve the narrower float. If you depended on `float64` output, add an explicit `.astype('float64')`.
</Warning>

These result-dtype rules — unary math promotion, reduction accumulator widening (`sum` of `int32` → `int64`), true division → float, and so on — are all mirrored in the return types. See [Compile-time dtype tracking](./typescript-patterns#compile-time-dtype-tracking).
