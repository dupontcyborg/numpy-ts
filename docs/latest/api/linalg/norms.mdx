...
```typescript
function linalg.det(a: ArrayLike): number | NDArray | Complex
```
...
**Returns:** `number | bigint | Complex | NDArray` -- Scalar for 2-D input, `NDArray` for higher-dimensional input. Integer arrays with a 64-bit accumulator return `bigint`.
