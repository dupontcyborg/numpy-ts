...
|`x`|`ArrayLike`|`undefined`|Values where condition is `true`. Scalars and array-likes accepted. Required if `y` is provided.|
|`y`|`ArrayLike`|`undefined`|Values where condition is `false`. Scalars and array-likes accepted. Required if `x` is provided.|
...
// Scalar branches — `x` and `y` accept plain numbers
const mask = np.where(np.greater(a, 3), 1, 0);
// array([0, 0, 0, 1, 1])

// `x = 0` / `y = 0` are now honored (previously fell through to the indices form)
const clipped = np.where(np.greater(a, 3), a, 0);
// array([0, 0, 0, 4, 5])
...
```typescript
function count_nonzero(a: ArrayLike, axis?: number | number[]): number | NDArray
```

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `a` | `ArrayLike` | -- | Input array. |
| `axis` | `number \| number[]` | `undefined` | Axis along which to count. If `undefined`, counts over the entire array. |
