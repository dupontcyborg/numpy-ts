...
<Note>
Changed in v1.7.0: integer input with `decimals >= 0` has nothing to round, so it now comes back as a view of the input rather than a copy. This matches NumPy. Writes to the result reach the input array, so call `.copy()` if you need an independent buffer. `base` is set on exactly the results that alias.
</Note>
