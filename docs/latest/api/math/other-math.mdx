...
Return the real part of each element. For real-valued arrays there is nothing to extract, so the input is handed back as a view.
...
<Note>
Changed in v1.7.0: a real-valued input now comes back as a view rather than a copy, matching NumPy. Writes to the result reach the input array. Complex input still allocates. Check `base` to tell the two apart -- it is set only when the result aliases.
</Note>
...
<Note>
Unlike `real`, this always allocates for real-valued input -- the zeros it returns are a fresh buffer with no `base`, so writing to the result never touches the input.
</Note>
