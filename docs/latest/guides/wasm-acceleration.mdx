...
| Kernel | Covers |
|-|-|
| `all_equal` | Whole-array equality, every dtype except `float16`, with `equal_nan` variants |
| `compare` | Element-wise comparisons |
| `rounding` | `round`, `around`, `rint`, `ceil`, `floor`, `trunc` |
| `isinf` | Infinity testing across dtypes |
| `lcm` | Least common multiple |
| `bulk_mem` | Shared `@memcpy` / `@memset` helpers used by `pad`, `tile` and `roll` |

`gcd` gained `uint32`, `int64` and `uint64` variants, and `tensordot` no longer runs its own contraction loop -- it transposes and reshapes to 2-D and delegates to the existing `matmul` kernel, the same strategy NumPy uses.
