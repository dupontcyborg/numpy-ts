...
```typescript
function copysign(x1: ArrayLike, x2: ArrayLike): NDArray
```
...
```typescript
function nextafter(x1: ArrayLike, x2: ArrayLike): NDArray
```
...
```typescript
function promote_types(type1: DType, type2: DType): DType
```

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `type1` | `DType` | -- | First dtype. |
| `type2` | `DType` | -- | Second dtype. |
