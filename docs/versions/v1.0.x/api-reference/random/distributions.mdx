...
All functions on this page are accessed via the `np.random` namespace and use the global MT19937 generator. The same distributions are also available on `Generator` instances (see [Generator API](/versions/v1.0.x/api-reference/random/generator)).
...
| `cov` | `ArrayLike` | -- | Covariance matrix (N x N, symmetric positive semi-definite). |
