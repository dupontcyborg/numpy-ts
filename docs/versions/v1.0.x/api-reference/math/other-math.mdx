...
```typescript
function clip(a: ArrayLike, a_min: number | null, a_max: number | null): NDArray
```
...
```typescript
function angle(z: ArrayLike, deg?: boolean): NDArray
```

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `z` | `ArrayLike` | - | Input array of complex numbers. |
