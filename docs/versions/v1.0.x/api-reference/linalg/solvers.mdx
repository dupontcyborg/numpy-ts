...
function linalg.lstsq(
  ...
  b: ArrayLike,
  rcond?: number
): [NDArray, NDArray, number, NDArray]
```
...
**Returns:** `[NDArray, NDArray, number, NDArray]` -- A tuple of:
...
- `sv` -- Singular values of `a` in descending order.
...
const [x, residuals, rank, sv] = np.linalg.lstsq(A, b);
