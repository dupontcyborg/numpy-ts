...
<Note>
`NDArrayCore` returned from `numpy-ts/core` still has properties like `.shape`, `.dtype`, `.T`, and `.toString()`. It only lacks the chainable operation methods (`.add()`, `.reshape()`, etc.). See the [Tree-Shaking guide](/versions/v1.0.x/guides/tree-shaking) for a full comparison.
</Note>
...
<CardGroup cols={3}>
  <Card title="Array Basics" icon="grid-2" href="/versions/v1.0.x/guides/array-basics">
  ...
  <Card title="Broadcasting" icon="arrows-maximize" href="/versions/v1.0.x/guides/broadcasting">
