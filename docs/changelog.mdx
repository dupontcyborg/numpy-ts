...
<Update label="v1.7.0" description="2026-08-29">

## numpy-ts v1.7.0

Performance release: new WASM SIMD kernels, lower per-call overhead, and the NumPy-compatibility fixes found along the way.

Measured like-for-like across the 2,388 benchmarks common to both sides, the geometric mean against NumPy improves from 0.87x to 0.77x. numpy-ts is now roughly **1.3x faster than NumPy** over the suite. Benchmarks slower than 5x NumPy fell from 48 to 25, and none remain above 10x on the standard suite (the large-array sweep still has 15). Coverage grew from 2,390 to 3,513 cases, sweeping every operation across all applicable dtypes.
...
* **`real`, `real_if_close`, `round`/`around` (integer input, `decimals >= 0`) and `apply_along_axis` now return views**, matching NumPy. Writes to a result or to an `apply_along_axis` callback argument. Call `.copy()` if you relied on the old copying behavior. `base` is set on exactly the results that alias, so it is a reliable test rather than a dtype guess. See [views and copies](/latest/guides/views-copies).
...
<Update label="v1.6.0" description="2026-07-16">

## numpy-ts v1.6.0

Compile-time `dtype` typing across the public API. `NDArray<D>` now tracks its dtype through nearly every operation at zero runtime cost, so results report their exact dtype (`add(int32Arr, float32Arr)` → `NDArray<'float64'>`) and element access is typed per dtype (`bigint` for `int64`/`uint64`, `Complex` for complex, `number` otherwise). See [Compile-time dtype tracking](/v1.6.x/guides/typescript-patterns#compile-time-dtype-tracking).
