...
canonical: https://numpyts.dev/latest/api/indexing/advanced-indexing
---
...
`vindex` closely corresponds to [NumPy's integer array indexing](https://numpy.org/doc/stable/user/basics.indexing.html#integer-array-indexing). It allows selection of arbitrary items in the array based on their N-dimensional index. Each integer array represents a number of indices into that dimension.
...
Normally `vindex` is called with `NDArray` or `number[]` objects, but you can also freely mix in any of the arguments `slice` accepts. The order of dimensions in the result matches [NumPy's behaviour](https://numpy.org/doc/stable/user/basics.indexing.html#combining-advanced-and-basic-indexing): if the advanced indices are contiguous, they replace in place; otherwise they are transposed to be the first dimensions.

---

### take

Take elements from an array along an axis. The output preserves the shape of `indices`: indexing with a 2-D index array produces an output whose indexed-axis is replaced by the index array's shape. A scalar index reduces rank by one along that axis (NumPy semantics). Legacy `number[]` callers are unchanged.
...
type ArrayLike = NDArrayCore | number | bigint | number[] | number[][] | number[][][];
```
...
| `indices` | `ArrayLike` | -- | Indices of the values to extract. Accepts NDArrays of any rank, nested `number[]`/`number[][]`/`number[][][]`, or a scalar. |
| `axis` | `number` | `undefined` | Axis along which to select values. When `undefined`, the input array is flattened before indexing. |

**Returns:** `NDArray` -- Array of values taken from `a`. Its shape is `[...a.shape[:axis], ...indices.shape, ...a.shape[axis+1:]]`.
...
// 2-D indices: output preserves the index shape along the indexed axis.
const src = np.array([10, 20, 30, 40, 50]);
const idx2D = np.array([[0, 2], [4, 1]]);
np.take(src, idx2D);
// array([[10, 30],
//        [50, 20]])
// shape [2, 2]

// Scalar index: rank reduces by one along the axis.
np.take(mat, 1, 0);
// array([3, 4])
// shape [2]
```
...
function select(
  ...
  defaultVal?: ArrayLike
): NDArray
```

Each element of `condlist` and `choicelist` is run through `asarray`, so plain `number[]`, `boolean[]`, nested arrays, and NDArrays are all accepted. `defaultVal` is broadcast to the output shape, allowing per-element fall-through values. Existing scalar/NDArray calls behave identically.

|-|-|-|-|
| Name | Type | Default | Description |
|-|-|-|-|
| `condlist` | `ArrayLike[]` | -- | List of boolean array-likes. The first condition that is true determines which choice is used. |
| `choicelist` | `ArrayLike[]` | -- | List of array-likes from which output elements are taken. Must be the same length as `condlist`. |
| `defaultVal` | `ArrayLike` | `0` | Value used when no condition is satisfied. Broadcasts to the output shape. |

**Returns:** `NDArray` -- Array where element `i` is taken from `choicelist[j][i]` for the first `j` where `condlist[j][i]` is true, or from `defaultVal` (broadcast) if no condition is true.
...
// Plain number[] choice and an array defaultVal that broadcasts per-element.
const fallback = np.array([100, 200, 300, 400, 500, 600]);
np.select(
  [np.less(x, 2)],
  [[0, 10, 20, 30, 40, 50]],
  fallback
);
// array([0, 10, 300, 400, 500, 600])
```
