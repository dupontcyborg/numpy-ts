...
canonical: https://numpyts.dev/latest/api/creation/from-data
---
...
```typescript
function meshgrid(...args: (ArrayLike | MeshgridOptions)[]): NDArray[]
...
|Name|Type|Default|Description|
|-|-|-|-|
|`...args`|`(ArrayLike \| MeshgridOptions)[]`|--|1-D arrays representing coordinates of a grid, optionally including a `MeshgridOptions` bag in any positional slot.|
|`indexing`|`'xy' \| 'ij'`|`'xy'`|Cartesian (`'xy'`) or matrix (`'ij'`) indexing of the output.|
|`sparse`|`boolean`|`false`|If `true`, return open (sparse) grids — one axis populated, others size 1 — for memory-efficient broadcasting.|
|`copy`|`boolean`|`true`|If `false`, return broadcast views instead of copying. Views are read-only-safe but share memory with the inputs.|
...
// Sparse (open) grids — broadcastable, much smaller in memory
const [Xs, Ys] = np.meshgrid(x, y, { sparse: true });
// Xs.shape = [1, 3], Ys.shape = [2, 1]

// Broadcast views — no copy
const [Xv, Yv] = np.meshgrid(x, y, { copy: false });

// Matrix indexing
const [Xi, Yi] = np.meshgrid(x, y, { indexing: 'ij' });
// Xi.shape = [3, 2], Yi.shape = [3, 2]
```
...
// Default is now 'xy' (NumPy-compatible)
const [X, Y] = meshgrid(x, y);
const Z = add(square(X), square(Y));

// Opt back into pre-v1.4.0 core behavior
const [Xi, Yi] = meshgrid(x, y, { indexing: 'ij' });
```
</CodeGroup>
