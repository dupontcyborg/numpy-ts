...
OPERATIONS: Dict[str, Callable[[Dict[str, Any]], Any]] = {
    ...
    "allclose": lambda a: bool(np.allclose(a["a"], a["b"])),
    "angle": lambda a: np.angle(a["a"]),
    "append": lambda a: np.append(a["a"], a["b"]),
    "apply_along_axis": lambda a: np.apply_along_axis(np.sum, 0, a["a"]),
    "apply_over_axes": lambda a: np.apply_over_axes(lambda x, ax: np.sum(x, axis=ax, keepdims=True), a["a"], [0]),
    "array_equal": lambda a: bool(np.array_equal(a["a"], a["b"])),
    "array_equiv": lambda a: bool(np.array_equiv(a["a"], a["b"])),
    "array_split": lambda a: list(np.array_split(a["a"], 3)),
    "atleast_1d": lambda a: np.atleast_1d(a["a"]),
    "atleast_2d": lambda a: np.atleast_2d(a["a"]),
    "atleast_3d": lambda a: np.atleast_3d(a["a"]),
    "broadcast_arrays": lambda a: list(np.broadcast_arrays(a["a"], a["b"])),
    "broadcast_shapes": lambda a: np.array(np.broadcast_shapes(a["shape"], (1, a["shape"][1])), dtype=np.float64),
    "ceil": lambda a: np.ceil(a["a"]),
    "choose": lambda a: np.choose(a["a"], [a["b"], a["b"], a["b"]]),
    "column_stack": lambda a: np.column_stack([a["a"], a["b"]]),
    "diag_indices_from": lambda a: list(np.diag_indices_from(a["a"])),
    "diagflat": lambda a: np.diagflat(a["a"]),
    "dsplit": lambda a: list(np.dsplit(a["a"], 2)),
    "dstack": lambda a: np.dstack([a["a"], a["b"]]),
    "equal": lambda a: np.equal(a["a"], a["b"]),
    "expand_dims": lambda a: np.expand_dims(a["a"], 0),
    "fill_diagonal": lambda a: (np.fill_diagonal(a["a"], 0), a["a"])[1],
    "fix": lambda a: np.fix(a["a"]),
    "fliplr": lambda a: np.fliplr(a["a"]),
    "flipud": lambda a: np.flipud(a["a"]),
    "floor": lambda a: np.floor(a["a"]),
    "fromfunction": lambda a: np.fromfunction(lambda i, j: i + j, a["shape"]),
    "fromiter": lambda a: np.fromiter(range(a["n"]), dtype=np.float64),
    "greater": lambda a: np.greater(a["a"], a["b"]),
    "greater_equal": lambda a: np.greater_equal(a["a"], a["b"]),
    "histogramdd": lambda a: np.histogramdd(a["a"]),
    "hsplit": lambda a: list(np.hsplit(a["a"], 2)),
    "imag": lambda a: np.imag(a["a"]),
    "insert": lambda a: np.insert(a["a"], 1, 99),
    "intersect1d": lambda a: np.intersect1d(a["a"], a["b"]),
    "isclose": lambda a: np.isclose(a["a"], a["b"]),
    "iscomplex": lambda a: np.iscomplex(a["a"]),
    "isin": lambda a: np.isin(a["a"], a["b"]),
    "ix_": lambda a: list(np.ix_(a["a"], a["b"])),
    "less": lambda a: np.less(a["a"], a["b"]),
    "less_equal": lambda a: np.less_equal(a["a"], a["b"]),
    "mask_indices": lambda a: list(np.mask_indices(a["n"], np.triu)),
    "meshgrid": lambda a: list(np.meshgrid(a["a"], a["b"])),
    "moveaxis": lambda a: np.moveaxis(a["a"], 0, 1),
    "nanargmax": lambda a: np.nanargmax(a["a"]),
    "nanargmin": lambda a: np.nanargmin(a["a"]),
    "nancumprod": lambda a: np.nancumprod(a["a"]),
    "nancumsum": lambda a: np.nancumsum(a["a"]),
    "nanmedian": lambda a: np.nanmedian(a["a"]),
    "nanprod": lambda a: np.nanprod(a["a"]),
    "nanstd": lambda a: np.nanstd(a["a"]),
    "nanvar": lambda a: np.nanvar(a["a"]),
    "nextafter": lambda a: np.nextafter(a["a"], a["b"]),
    "not_equal": lambda a: np.not_equal(a["a"], a["b"]),
    "place": lambda a: (np.place(a["a"], a["b"], a["c"]), a["a"])[1],
    "put": lambda a: (np.put(a["a"], a["indices"], a["c"]), a["a"])[1],
    "put_along_axis": lambda a: (np.put_along_axis(a["a"], a["c"].astype(np.intp), a["c"], 0), a["a"])[1],
    "putmask": lambda a: (np.putmask(a["a"], a["b"], a["c"]), a["a"])[1],
    "real": lambda a: np.real(a["a"]),
    "real_if_close": lambda a: np.real_if_close(a["a"]),
    "resize": lambda a: np.resize(a["a"], a["new_shape"]),
    "rint": lambda a: np.rint(a["a"]),
    "rollaxis": lambda a: np.rollaxis(a["a"], 1),
    "round": lambda a: np.round(a["a"]),
    "select": lambda a: np.select([a["b"]], [a["a"]]),
    "setdiff1d": lambda a: np.setdiff1d(a["a"], a["b"]),
    "setxor1d": lambda a: np.setxor1d(a["a"], a["b"]),
    "spacing": lambda a: np.spacing(a["a"]),
    "split": lambda a: list(np.split(a["a"], 2)),
    "tril_indices_from": lambda a: list(np.tril_indices_from(a["a"])),
    "triu_indices_from": lambda a: list(np.triu_indices_from(a["a"])),
    "trunc": lambda a: np.trunc(a["a"]),
    "union1d": lambda a: np.union1d(a["a"], a["b"]),
    "unique": lambda a: np.unique(a["a"]),
    "unique_all": lambda a: list(np.unique_all(a["a"])),
    "unique_inverse": lambda a: list(np.unique_inverse(a["a"])),
    "vsplit": lambda a: list(np.vsplit(a["a"], 2)),
    "vander": lambda a: np.vander(a["a"]),
