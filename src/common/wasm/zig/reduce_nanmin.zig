//! WASM reduction nanmin kernels for float types only.
//!
...
test "reduce_nanmin_f64 basic" {
    const testing = @import("std").testing;
    const nan = @as(f64, @bitCast(@as(u64, 0x7FF8000000000000)));
    const a = [_]f64{ 5.0, nan, 1.0, nan, 3.0 };
    try testing.expectApproxEqAbs(reduce_nanmin_f64(&a, 5), 1.0, 1e-10);
}

test "reduce_nanmin_f64 all NaN returns +inf" {
    const testing = @import("std").testing;
    const nan = @as(f64, @bitCast(@as(u64, 0x7FF8000000000000)));
...
test "reduce_nanmin_f64 no NaN same as min" {
    ...
    try testing.expectApproxEqAbs(reduce_nanmin_f64(&a, 4), 1.0, 1e-10);
}

test "reduce_nanmin_f64 NaN at first and last" {
    const testing = @import("std").testing;
    const nan = @as(f64, @bitCast(@as(u64, 0x7FF8000000000000)));
    const a = [_]f64{ nan, 8.0, 2.0, nan };
    try testing.expectApproxEqAbs(reduce_nanmin_f64(&a, 4), 2.0, 1e-10);
}

test "reduce_nanmin_f64 negatives" {
    const testing = @import("std").testing;
    const nan = @as(f64, @bitCast(@as(u64, 0x7FF8000000000000)));
    const a = [_]f64{ -1.0, nan, -5.0, -3.0 };
    try testing.expectApproxEqAbs(reduce_nanmin_f64(&a, 4), -5.0, 1e-10);
}

test "reduce_nanmin_f32 basic" {
    const testing = @import("std").testing;
    const nan = @as(f32, @bitCast(@as(u32, 0x7FC00000)));
    const a = [_]f32{ 7.0, nan, 2.0, nan };
    try testing.expectApproxEqAbs(reduce_nanmin_f32(&a, 4), 2.0, 1e-5);
}

test "reduce_nanmin_f32 all NaN returns +inf" {
    const testing = @import("std").testing;
    const nan = @as(f32, @bitCast(@as(u32, 0x7FC00000)));
