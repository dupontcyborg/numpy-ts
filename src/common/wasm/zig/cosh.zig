//! WASM element-wise cosh kernels for float / int types.
//!
...
/// cosh for a 2-wide f64 lane via the shared sinh/cosh core.
inline fn coshv(x: simd.V2f64) simd.V2f64 {
...
/// Element-wise cosh for f64 using 2-wide SIMD.
export fn cosh_f64(a: [*]const f64, out: [*]f64, N: u32) void {
    const n_simd = N & ~@as(u32, 1);
    var i: u32 = 0;
    while (i < n_simd) : (i += 2) {
        simd.store2_f64(out, i, coshv(simd.load2_f64(a, i)));
    }
    while (i < N) : (i += 1) {
...
/// Element-wise cosh for f32 via the 2-wide f64 core, then narrowed.
...
inline fn coshInt_f32(comptime I: type, a: [*]const I, out: [*]f32, N: u32) void {
    const n_simd = N & ~@as(u32, 1);
    var i: u32 = 0;
    while (i < n_simd) : (i += 2) {
        const vi = @as(*align(1) const @Vector(2, I), @ptrCast(a + i)).*;
