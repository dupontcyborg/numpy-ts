//! WASM element-wise bitwise XOR kernels for all integer types.
//!
...
/// Element-wise bitwise XOR for i64 using 2-wide SIMD.
/// Handles both signed (i64) and unsigned (u64).
export fn bitwise_xor_i64(a: [*]const i64, b: [*]const i64, out: [*]i64, N: u32) void {
...
/// Element-wise bitwise XOR for i32 using 4-wide SIMD.
/// Handles both signed (i32) and unsigned (u32).
export fn bitwise_xor_i32(a: [*]const i32, b: [*]const i32, out: [*]i32, N: u32) void {
...
/// Element-wise bitwise XOR for i16 using 8-wide SIMD.
/// Handles both signed (i16) and unsigned (u16).
export fn bitwise_xor_i16(a: [*]const i16, b: [*]const i16, out: [*]i16, N: u32) void {
...
/// Element-wise bitwise XOR for i8 using 16-wide SIMD.
/// Handles both signed (i8) and unsigned (u8).
export fn bitwise_xor_i8(a: [*]const i8, b: [*]const i8, out: [*]i8, N: u32) void {
...
test "bitwise_xor_i32 basic" {
    const testing = @import("std").testing;
    const a = [_]i32{ 0xFF, 0xFF, 0xAA, 0x55, 0x00 };
    const b = [_]i32{ 0xFF, 0x00, 0x55, 0xAA, 0x00 };
    var out: [5]i32 = undefined;
    bitwise_xor_i32(&a, &b, &out, 5);
    try testing.expectEqual(out[0], 0x00);
...
test "bitwise_xor_i8 SIMD boundary N=17" {
    const testing = @import("std").testing;
