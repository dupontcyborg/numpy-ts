...
/// Element-wise logical XOR for i64 — 2-wide.
...
/// Element-wise logical XOR against a scalar for i64 — 2-wide.
export fn logical_xor_scalar_i64(a: [*]const i64, out: [*]u8, N: u32, scalar: i64) void {
    const sb = scalar != 0;
    const sv: @Vector(2, bool) = @splat(sb);
    const n2 = N & ~@as(u32, 1);
    var i: u32 = 0;
    while (i < n2) : (i += 2) store2(out, i, truthy2(a, i) != sv);
    while (i < N) : (i += 1) out[i] = @intFromBool((a[i] != 0) != sb);
}
