    ...
    while (ii < M) : (ii += TILE_F64) {
        ...
        while (jj < N) : (jj += TILE_F64) {
            ...
            while (kk < K) : (kk += TILE_F64) {
                ...
                // 4×8 micro-kernel (16 accumulators)
                var i: usize = ii;
                while (i + 4 <= i_end) : (i += 4) {
                    var j: usize = jj;
                    while (j + 8 <= j_end) : (j += 8) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            c00 = simd.mulAdd_f64x2(a0, b0, c00);
                            c01 = simd.mulAdd_f64x2(a0, b1, c01);
                            c02 = simd.mulAdd_f64x2(a0, b2, c02);
                            c03 = simd.mulAdd_f64x2(a0, b3, c03);
                            const a1: simd.V2f64 = @splat(a[(i + 1) * K + k]);
                            c10 = simd.mulAdd_f64x2(a1, b0, c10);
                            c11 = simd.mulAdd_f64x2(a1, b1, c11);
                            c12 = simd.mulAdd_f64x2(a1, b2, c12);
                            c13 = simd.mulAdd_f64x2(a1, b3, c13);
                            const a2: simd.V2f64 = @splat(a[(i + 2) * K + k]);
                            c20 = simd.mulAdd_f64x2(a2, b0, c20);
                            c21 = simd.mulAdd_f64x2(a2, b1, c21);
                            c22 = simd.mulAdd_f64x2(a2, b2, c22);
                            c23 = simd.mulAdd_f64x2(a2, b3, c23);
                            const a3: simd.V2f64 = @splat(a[(i + 3) * K + k]);
                            c30 = simd.mulAdd_f64x2(a3, b0, c30);
                            c31 = simd.mulAdd_f64x2(a3, b1, c31);
                            c32 = simd.mulAdd_f64x2(a3, b2, c32);
                            c33 = simd.mulAdd_f64x2(a3, b3, c33);
                        }
                    ...
                    while (j + 4 <= j_end) : (j += 4) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            const a0: simd.V2f64 = @splat(a[(i + 0) * K + k]);
                            r00 = simd.mulAdd_f64x2(a0, b0, r00);
                            r01 = simd.mulAdd_f64x2(a0, b1, r01);
                            const a1: simd.V2f64 = @splat(a[(i + 1) * K + k]);
                            r10 = simd.mulAdd_f64x2(a1, b0, r10);
                            r11 = simd.mulAdd_f64x2(a1, b1, r11);
                            const a2: simd.V2f64 = @splat(a[(i + 2) * K + k]);
                            r20 = simd.mulAdd_f64x2(a2, b0, r20);
                            r21 = simd.mulAdd_f64x2(a2, b1, r21);
                            const a3: simd.V2f64 = @splat(a[(i + 3) * K + k]);
                            r30 = simd.mulAdd_f64x2(a3, b0, r30);
                            r31 = simd.mulAdd_f64x2(a3, b1, r31);
                        }
                        inline for (0..4) |r| {
                            const off = (i + r) * N + j;
                            const rv = switch (r) {
                    ...
                    while (j + 2 <= j_end) : (j += 2) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            s0 = simd.mulAdd_f64x2(@as(simd.V2f64, @splat(a[(i + 0) * K + k])), bv, s0);
                            s1 = simd.mulAdd_f64x2(@as(simd.V2f64, @splat(a[(i + 1) * K + k])), bv, s1);
                            s2 = simd.mulAdd_f64x2(@as(simd.V2f64, @splat(a[(i + 2) * K + k])), bv, s2);
                            s3 = simd.mulAdd_f64x2(@as(simd.V2f64, @splat(a[(i + 3) * K + k])), bv, s3);
                        }
                        inline for (0..4) |r| {
                            const off = (i + r) * N + j;
                            const sv = switch (r) {
                ...
                while (i < i_end) : (i += 1) {
                    ...
                    while (j + 4 <= j_end) : (j += 4) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            a0 = simd.mulAdd_f64x2(av, simd.load2_f64(b, k * N + j), a0);
                            a1 = simd.mulAdd_f64x2(av, simd.load2_f64(b, k * N + j + 2), a1);
                        }
                    ...
                    while (j + 2 <= j_end) : (j += 2) {
                        ...
                        var k: usize = kk;
                        while (k < k_end) : (k += 1) {
                            acc = simd.mulAdd_f64x2(@as(simd.V2f64, @splat(a[i * K + k])), simd.load2_f64(b, k * N + j), acc);
                        }
                ...
                while (i + 4 <= i_end) : (i += 4) {
                    ...
                    while (j + 16 <= j_end) : (j += 16) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            c00 = simd.mulAdd_f32x4(a0, b0, c00);
                            c01 = simd.mulAdd_f32x4(a0, b1, c01);
                            c02 = simd.mulAdd_f32x4(a0, b2, c02);
                            c03 = simd.mulAdd_f32x4(a0, b3, c03);
                            const a1: simd.V4f32 = @splat(a[(i + 1) * K + k]);
                            c10 = simd.mulAdd_f32x4(a1, b0, c10);
                            c11 = simd.mulAdd_f32x4(a1, b1, c11);
                            c12 = simd.mulAdd_f32x4(a1, b2, c12);
                            c13 = simd.mulAdd_f32x4(a1, b3, c13);
                            const a2: simd.V4f32 = @splat(a[(i + 2) * K + k]);
                            c20 = simd.mulAdd_f32x4(a2, b0, c20);
                            c21 = simd.mulAdd_f32x4(a2, b1, c21);
                            c22 = simd.mulAdd_f32x4(a2, b2, c22);
                            c23 = simd.mulAdd_f32x4(a2, b3, c23);
                            const a3: simd.V4f32 = @splat(a[(i + 3) * K + k]);
                            c30 = simd.mulAdd_f32x4(a3, b0, c30);
                            c31 = simd.mulAdd_f32x4(a3, b1, c31);
                            c32 = simd.mulAdd_f32x4(a3, b2, c32);
                            c33 = simd.mulAdd_f32x4(a3, b3, c33);
                        }
                    ...
                    while (j + 8 <= j_end) : (j += 8) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            const a0: simd.V4f32 = @splat(a[(i + 0) * K + k]);
                            r00 = simd.mulAdd_f32x4(a0, b0, r00);
                            r01 = simd.mulAdd_f32x4(a0, b1, r01);
                            const a1: simd.V4f32 = @splat(a[(i + 1) * K + k]);
                            r10 = simd.mulAdd_f32x4(a1, b0, r10);
                            r11 = simd.mulAdd_f32x4(a1, b1, r11);
                            const a2: simd.V4f32 = @splat(a[(i + 2) * K + k]);
                            r20 = simd.mulAdd_f32x4(a2, b0, r20);
                            r21 = simd.mulAdd_f32x4(a2, b1, r21);
                            const a3: simd.V4f32 = @splat(a[(i + 3) * K + k]);
                            r30 = simd.mulAdd_f32x4(a3, b0, r30);
                            r31 = simd.mulAdd_f32x4(a3, b1, r31);
                        }
                        inline for (0..4) |r| {
                            const off = (i + r) * N + j;
                            const rv = switch (r) {
                    ...
                    while (j + 4 <= j_end) : (j += 4) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            s0 = simd.mulAdd_f32x4(@as(simd.V4f32, @splat(a[(i + 0) * K + k])), bv, s0);
                            s1 = simd.mulAdd_f32x4(@as(simd.V4f32, @splat(a[(i + 1) * K + k])), bv, s1);
                            s2 = simd.mulAdd_f32x4(@as(simd.V4f32, @splat(a[(i + 2) * K + k])), bv, s2);
                            s3 = simd.mulAdd_f32x4(@as(simd.V4f32, @splat(a[(i + 3) * K + k])), bv, s3);
                        }
                        inline for (0..4) |r| {
                            const off = (i + r) * N + j;
                            const sv = switch (r) {
                ...
                while (i < i_end) : (i += 1) {
                    ...
                    while (j + 8 <= j_end) : (j += 8) {
                        ...
                        while (k < k_end) : (k += 1) {
                            ...
                            a0 = simd.mulAdd_f32x4(av, simd.load4_f32(b, k * N + j), a0);
                            a1 = simd.mulAdd_f32x4(av, simd.load4_f32(b, k * N + j + 4), a1);
                        }
                    ...
                    while (j + 4 <= j_end) : (j += 4) {
                        ...
                        var k: usize = kk;
                        while (k < k_end) : (k += 1) {
                            acc = simd.mulAdd_f32x4(@as(simd.V4f32, @splat(a[i * K + k])), simd.load4_f32(b, k * N + j), acc);
                        }
