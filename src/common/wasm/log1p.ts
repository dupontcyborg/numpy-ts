...
import * as log1pBase from './bins/log1p.wasm';
import * as log1pRelaxed from './bins/log1p-relaxed.wasm';
...
let _bins: typeof log1pBase | null = null;
function bins(): typeof log1pBase {
  _bins ??= useRelaxedKernels() ? log1pRelaxed : log1pBase;
  return _bins;
}

const kernels: Partial<Record<DType, UnaryFn>> = {
  float64: (...a) => bins().log1p_f64(...a),
  float32: (...a) => bins().log1p_f32(...a),
};

// Large int → f64 output (i32/u32/i64/u64 need f64 precision)
const largeIntKernels: Partial<Record<DType, UnaryFn>> = {
  int64: (...a) => bins().log1p_i64_f64(...a),
  uint64: (...a) => bins().log1p_u64_f64(...a),
  int32: (...a) => bins().log1p_i32_f64(...a),
  uint32: (...a) => bins().log1p_u32_f64(...a),
};

// Small int → f32 output (i8/u8/i16/u16 → f32, then optionally downcast to f16)
const smallIntKernels: Partial<Record<DType, UnaryFn>> = {
  int16: (...a) => bins().log1p_i16_f32(...a),
  uint16: (...a) => bins().log1p_u16_f32(...a),
  int8: (...a) => bins().log1p_i8_f32(...a),
  uint8: (...a) => bins().log1p_u8_f32(...a),
};
 ...
 */
export function wasmLog1p(a: ArrayStorage): ArrayStorage | null {
  ...
  if (dtype === 'float16') {
    ...
    bins().log1p_f32(aPtr, outRegion.ptr, size);
