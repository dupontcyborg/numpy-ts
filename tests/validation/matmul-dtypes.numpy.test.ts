...
describe('NumPy Validation: matmul across all dtypes', () => {
  beforeAll(() => {
  ...
  describe('larger matrices (10x10)', () => {
    ...
    for (const dtype of DTYPES) {
      it(`${dtype}: 10x10 @ 10x10 matches NumPy`, () => {
        ...
        const a = np.remainder(np.arange(0, 100, 1, dtype).reshape([10, 10]), np.array([5], dtype));
        const b = np.remainder(np.arange(0, 100, 1, dtype).reshape([10, 10]), np.array([3], dtype));
